...
# Mayponds=number of May ponds estimated from PCHM
...
# Autucorrelation and partial autocorrelation for PCHM-estimated number of May ponds, Fig. S1:
par(mfrow=c(1,3))
PCHM <- ts(Mayponds)
...
# Autocorrelations for bird occurrences, Fig. S2:
par(mfrow=c(3,2))
...
# Autocorrelations for bird abundances, Fig. S3:
par(mfrow=c(3,2))
...
# Partial autocorrelations for bird occurrences, Fig. S4:
par(mfrow=c(3,2))
stopAMCO <- ts(logstpAMCO)
stopPBGR <- ts(logstpPBGR)
stopSORA <- ts(logstpSORA)
...
# Partial autocorrelations for bird abundances, Fig. S5:
par(mfrow=c(3,2))
birdAMCO <- ts(logbrdAMCO)
birdPBGR <- ts(logbrdPBGR)
birdSORA <- ts(logbrdSORA)
...
# Cross-correlations for bird occurrences, Fig. S6:
par(mfrow=c(3,2))
...
# Cross-correlations for bird abundances, Fig. S7:
par(mfrow=c(3,2))
...
# Figs. S6 & S7 clearly show the positive relationship between wetlands and
...
#-------  Stops/Occurrences -----------------------------------------
d = read.xls("bird_data_by_block_Final.xlsx",sheet="birds and ponds")
...
maypond.pred = read.xls("downscaled_climate.xlsx", sheet="downscaled")
